...
func main() {
	...
	for {
		...
		temp := int([]byte(ch)[0])  // 就是字符对应的 ascii
		if operStack.IsOper(temp) { // 说明是符号
			...
			if operStack.Top == -1 { // 空栈
				...
				// 如果发现 operStack 栈顶的运算符的优先级大于等于当前准备
				// 入栈的运算符的优先级，就从符号栈 pop 出，并从数栈也 pop
				// 两个数，进行运算，运算后的结果再重新入栈到数栈，符号再入符号栈
			...
			keepNum += ch
			// 2. 每次要向 index 的后面字符测试一下，看看是不是运算符

			// 如果已经到表达最后， 直接将 keepNum
			if index == len(exp)-1 {
				val, _ := strconv.ParseInt(keepNum, 10, 64)
				numStack.Push(int(val)) // 3 ==> 51 错误
			} else {
	...
	// 如果我们的算法没有问题、表达式也是正确的，则结果就是 numStack最后数
	res, _ := numStack.Pop()
